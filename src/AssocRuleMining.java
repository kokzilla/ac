...
/**
 * Set of utillities to support various Association Rule Mining (ARM) algorithms
...
public class AssocRuleMining extends JFrame {
	...
	/**
	 * 2-D aray to hold input data from data file. Note that within the data array
	 * records are numbered from zero, thus record one has index 0 etc.
	 * <P>
	 * First index is row (record or TID) number starting from 0, and second is
	 * attribute (column) number starting from zero.
	 */
	protected short[][] dataArray = null;
	/**
	 * 2-D array used to renumber columns for input data in terms of frequency of
	 * single attributes (reordering will enhance performance for some ARM
	 * algorithms).
	 */
	...
	/**
	 * Command line argument for file name for testset (used in classification where
	 * separate test and training set files may be used.
	 */
	protected String testSetFileName = null;
	/**
	 * Command line argument for number of columns (attributes) in input data.
	 */
	 ...
	 * <P>
	 * Set when input data is read and the number of records is known, reset if
	 * input data is resized so that only N percent is used.
	 */
	 ...
	 * 
	 * @param args
	 *            the command line arguments (array of String instances).
	 */
	...
	/**
	 * One argument constructior with argument from existing instance of class
	 * AssocRuleMining.
	 * 
	 * @param armInstance
	 *            the given instance of the <TT>AssocRuleMining</TT> class.
	 */
	...
	/**
	 * Default constructor used by: (1) BruteForce, (2) Total Support Tree, (3)
	 * ClassAppMapGUI and (4) AprioriTgui classes and others.
	 */
	...
	/* IDENTIFY ARGUMENT */
	/**
	 * Identifies nature of individual command line agruments: -C = confidence, -F =
	 * file name, -S = support.
	 */

	protected void idArgument(String argument) {
		if (argument.length() < 3) {
			JOptionPane.showMessageDialog(null, "Command line argument \"" + argument + "\" too short.",
			...
			default:
				JOptionPane.showMessageDialog(null,
						"Unrecognise command " + "line  argument: \"" + flag + argument + "\"'.",
						"COMMAND LINE INPUT ERROR", JOptionPane.ERROR_MESSAGE);
				errorFlag = false;
			}
		} else {
			JOptionPane.showMessageDialog(null,
					"All command line arguments " + "must commence with a '-' character ('" + argument + "')",
					"COMMAND LINE INPUT ERROR", JOptionPane.ERROR_MESSAGE);
			errorFlag = false;
		}
	}
	...
	/**
	 * Checks support and confidence input % values, if either is out of bounds then
	 * <TT>errorFlag</TT> set to <TT>false</TT>.
	 */
	...
	protected void checkSupportAndConfidence() {
		...
		if ((support < MIN_SUPPORT) || (support > MAX_SUPPORT)) {
			JOptionPane.showMessageDialog(null,
					"Support must be specified " + "as a percentage (" + MIN_SUPPORT + " - " + MAX_SUPPORT + ")",
		...
		if ((confidence < MIN_CONFIDENCE) || (confidence > MAX_CONFIDENCE)) {
			JOptionPane.showMessageDialog(null, "Confidence must be " + "specified as a percentage (" + MIN_CONFIDENCE
					+ " - " + MAX_CONFIDENCE + ")", "INPUT ERROR", JOptionPane.ERROR_MESSAGE);
			errorFlag = false;
		}
	}
	...
	protected void checkFileName() {
		if (fileName == null) {
			JOptionPane.showMessageDialog(null, "Must specify file name (-F)", "COMMAND LINE INPUT ERROR",
	...
	public void inputDataSet() {
		...
		if (inputFormatOkFlag) {
			if (checkOrdering()) {
				...
				JOptionPane.showMessageDialog(null, "Unknown error reading " + "file: " + fileName + "\n",
	 ...
	 * <TT>fileName</TT> (GUI version also exists).
	 * <P>
	 * Note that it is assumed that no empty records are included. Proceeds as
	 * follows:
	 * <OL>
	 * <LI>Gets number of rows (lines) in file, checking format of each line (space
	...
	protected void checkLine(int counter, String str) {
		...
		for (int index = 0; index < str.length(); index++) {
			if (!Character.isDigit(str.charAt(index)) && !Character.isWhitespace(str.charAt(index))) {
				JOptionPane.showMessageDialog(null, "Character on line " + counter + " is not a digit or white space",
						"FILE INPUT ERROR", JOptionPane.ERROR_MESSAGE);
	...
	/**
	 * Reads input data segment from a given file and places content into to the
	 * data array structure commencing at the given row index, continues until the
	 * end index is rerached.
	 * 
	 * @param fName
	 *            the given file name.
	 * @param startRowIndex
	 *            the given row strat index.
	 * @param endRowIndex
	 *            the given row end index.
	 */
	...
	/**
	 * Processes a line from the input file and places it in the <TT>dataArray</TT>
	 * structure.
	 * 
	 * @param line
	 *            the line to be processed from the input file
	 * @param rowIndex
	 *            the index to the current location in the <TT>dataArray</TT>
	 *            structure.
	...
	protected boolean checkLineOrdering(int lineNum, short[] itemSet) {
		for (int index = 0; index < itemSet.length - 1; index++) {
			if (itemSet[index] >= itemSet[index + 1]) {
				JOptionPane.showMessageDialog(null, "Attribute data in line " + lineNum + " not in numeric order",
						"FILE INPUT ERROR", JOptionPane.ERROR_MESSAGE);
				return (false);
			}
		}
	...
	protected void openFileName(String nameOfFile) {
		try {
			...
			JOptionPane.showMessageDialog(null, "Error Opening File \"" + nameOfFile + "\"", "FILE INPUT ERROR",
					JOptionPane.ERROR_MESSAGE);
			System.exit(1);
		}
	}
	...
	protected void openFilePath() {
		try {
			...
			JOptionPane.showMessageDialog(null, "Error Opening File \"" + filePath + "\"", "FILE INPUT ERROR",
					JOptionPane.ERROR_MESSAGE);
			System.exit(1);
		}
	}
	...
	protected void closeFile() {
		if (fileInput != null) {
			try {
				...
				JOptionPane.showMessageDialog(null, "Error Closeing File", "FILE ERROR", JOptionPane.ERROR_MESSAGE);
				System.exit(1);
			}
		}
	}
	...
	/**
	 * Commences process of reading indicated output schema file and stores to 1-D
	 * array.
	 * 
	 * @param textArea
	 *            the text area in the GUI used for output.
	 * @param fName
	...
	public void inputOutputSchema(JTextArea textArea, File fName) {
		...
		try {
			...
			textArea.append(
					"Number of lines (attributes) in output schema " + "file = " + numRowsInOutputSchema + "\n");
			...
			JOptionPane.showMessageDialog(null, "Error reading File", "FILE INPUT ERROR", JOptionPane.ERROR_MESSAGE);
	...
	/**
	 * Gets the number of lines (attributes) in the output schema file.
	 * <P>
	 * Similar to getNmberOfLines method above but without line checking.
	 * 
	 * @param fName
	 *            the name of the output schema file.
	 ...
	 * attributes in input file.
	 * <P>
	 * If either the output schema or the input file has nor been loaded then method
	 * will return false.
	 * 
	...
	public boolean checkSchemaVdata() {
		...
		if (outputSchema == null) {
			JOptionPane.showMessageDialog(null, "No output schema file.", "CHECK SCHEMA v DATA ATTRIBUTES ERROR",
					JOptionPane.ERROR_MESSAGE);
		...
		// Check data array
		if (dataArray == null) {
			JOptionPane.showMessageDialog(null, "No input data file.", "CHECK SCHEMA v DATA ATTRIBUTES ERROR",
					JOptionPane.ERROR_MESSAGE);
			return (!schemaAndDataAttsSame);
		}

		// Check lengths.
		if (outputSchema.length == numCols)
			return (schemaAndDataAttsSame);
		else {
			JOptionPane.showMessageDialog(null,
					"Number of attributes in " + "schema file (" + outputSchema.length + ") not\n"
							+ "same as number of attributes in data file (" + numCols + ")\n",
					"CHECK SCHEMA v DATA ATTRIBUTES ERROR", JOptionPane.ERROR_MESSAGE);
			return (!schemaAndDataAttsSame);
		}
	}
	 ...
	 * </PRE>
	 * 
	 * This would produce a countArray (ignore index 0 because there is no attributr
	 * number 0):
	 * 
	 * <PRE>
	 +---+---+---+---+---+---+
	 ...
	 * </PRE>
	 * 
	 * Note that the first row gives the new attribute number (old attribute number
	 * is the index). The second row here are the counts used to identify the
	 * ordering but which now no longer play a role in the conversion exercise. Thus
	 * the new column (attriburte) number for column/attribute 1 is column 3 (i.e.
	 * the first vale at index 1). The reconversion array will be of the form
	 * (values are the indexes from the conversion array while indexes represent the
	 * first vlaue from the conversion array):
	 * 
	 * <PRE>
	 +---+---+---+---+---+---+
	 |   | 2 | 5 | 1 | 3 | 4 |
	 ...
	 * </PRE>
	 * 
	 * For example to convert the attribute number 3 back to its original number we
	 * look up the value at index 3.
	 */
	 ...
	 * 
	 * @return 2-D array where first row represents column numbers and second row
	 *         represents support counts.
	 */
	 ...
	 * <TT>countSingles</TT> method so that array is ordered according to frequency
	 * of single items.
	 * <P>
	 * Used when ordering classification input data where we wish classes to be
	 * listed last.
	 * 
	 * @param countArray
	 *            The 2-D array returned by the <TT>countSingles</TT> method.
	...
	/**
	 * Defines conversion and reconversion arrays using only the first N elements of
	 * the count array, last elements are not ordered.
	 * 
	 * @param countArray
	 *            The 2-D array sorted by the <TT>orderCcountArray</TT> method.
	 * @param endIndex
	...
	/**
	 * Recasts the contents of the (input) data array so that each record is ordered
	 * according to conversion array.
	 * <P>
	 * Proceed as follows:
	 * 
	 * 1) For each record in the data array. Create an empty new itemSet array. 2)
	 * Place into this array attribute/column numbers that correspond to the
	 * appropriate equivalents contained in the conversion array. 3) Reorder this
	 * itemSet and return into the data array.
	 */
	 ...
	 * 
	 * 1) For each record in the data array. Create an empty new itemSet array. 2)
	 * Place into this array any column numbers in record that are supported at the
	 * index contained in the conversion array. 3) Assign new itemSet back into to
	 * data array
	 */
	...
	/**
	 * Gets number of supported single item sets (note this is not necessarily the
	 * same as the number of columns/attributes in the input set).
	 * 
	 ...
	 * 
	 * @param percentage
	 *            the percentage of the current input data that is to form the new
	 *            input data set (number between 0 and 100).
	 */
	...
	/**
	 * Converts input data from horizontal format to vertical format.
	 * <P>
	 ...
	 * index) the column/attribute number. WARNINGS: (1) Assumes that no input data
	 * reordering or pruning has been implemented, (2) Original dataset is deleted.
	 */
	...
	public void horizontal2vertical() {
		...
		for (int tidIndex = 0; tidIndex < dataArray.length; tidIndex++) {
			...
			if (dataArray[tidIndex] != null) {
				for (int dataIndex = 0; dataIndex < dataArray[tidIndex].length; dataIndex++) {
	...
	/**
	 * Dimensions new data array when converting from horizontal to vertical format.
	 * 
	...
	/**
	 * Horizontally segements the input data set into N segements and stores to disk
	 * using file names made up of the input file nam,e plus the segment number.
	 * 
	 * @param numSegments
	 *            the number of segments into which the data is to be decompossed.
	 */
	...
	public void segmentDataSet(int numSegments) throws IOException {
		...
		String shortFileName = fileName.substring(fileNameIndex + 1, fileName.length());
	...
	/**
	 * Reads input data segment by segement and stores segment in memory which is
	 * then partitioned and written to file.
	 * 
	 * @param numSegments
	 *            the number of segments into which the data is to be decompossed.
	 */
	...
	/**
	 * Vertically partitions, one partition per attribute, and stores to disk using
	 * file names made up of the input file name plus column numbers.
	 * 
	 * @param fName
	 *            the name of the file data is to be stored in.
	 */
	...
	protected int calcRowsPerSegment(int numSegments) {
		...
		if (rowsPerSegment == 0) {
			System.out.println("DATA SEGMENTATION ERROR: number of desired " + "segments (" + numSegments
					+ ") exceeds number of records (" + numRows + ") in input dataset");
			System.exit(1);
		}
	...
	/*
	 * Methods for inserting rules into a linked list of rules ordered according to
	 * confidence (most confident first). Each rule described in terms of 3 fields:
	 * 1) Antecedent (an item set), 2) a consequent (an item set), 3) a confidence
	 * value (double). <P>The support field is not used.
	 */
	...
	/**
	 * Inserts an (association/classification) rule into the bianry tree of rules
	 * pointed at by <TT>startRulelist</TT>.
	 * <P>
	 * The tree is ordered from left to right so that rules with highest "ordering
	 * value" (this is assumed to be a confidence value but could equaly well be
	 * some other value such as lift) are listed in the left most branch. If two
	 * rules have the same ordering value the new rule will be placed after the
	 * existing rule. Thus, if using an Apriori approach to generating rules, more
	 * general rules will appear first in the list with more specific rules (i.e.
	 * rules with a larger antecedent) appearing later as the more general rules
	 ...
	 * @param ordValue1
	 *            the primary ordering value (usually the confidence value).
	 * @param ordValue2
	 *            the secondary ordering value (usually the support value).
	 */

	protected void insertRuleIntoRulelist(short[] antecedent, short[] consequent, double ordValue1, double ordValue2) {
		// System.out.print("antecedent = ");
		// outputItemSet(antecedent);
		// System.out.print(", consequent = ");
		// outputItemSet(consequent);
		// System.out.println(", confidence = " + ordValue1 + ", support = " +
		// ordValue2);
		...
		if (startRulelist == null)
			startRulelist = new RuleNode(antecedent, consequent, ordValue1, ordValue2);
		// Otherwise "walk" tree
		else
			insertRuleIntoRulelist(startRulelist, antecedent, consequent, ordValue1, ordValue2);
	 ...
	 * pointed at by <TT>startRulelist</TT>.
	 * <P>
	 * Version which uses only one ordinal value, second ordinal value set to 0.0.
	 * 
	 * @param antecedent
	 *            the antecedent (LHS) of the rule.
	 * @param consequent
	...
	protected void insertRuleIntoRulelist(short[] antecedent, short[] consequent, double ordValue1) {
	 ...
	 * 
	 * @param node
	 *            the current location in the bin tree.
	 * @param antecedent
	 *            the antecedent (LHS) of the rule.
	 * @param consequent
	 *            the consequent (RHS) of the rule.
	...
	private void insertRuleIntoRulelist(RuleNode node, short[] antecedent, short[] consequent, double ordValue1,
			double ordValue2) {

		// Calculate selector
		boolean prcDwnLftBrnch = insertRuleSelector(node, antecedent, ordValue1, ordValue2);

		// Left branch
		if (prcDwnLftBrnch) {
			if (node.leftBranch == null)
				node.leftBranch = new RuleNode(antecedent, consequent, ordValue1, ordValue2);
			else
				insertRuleIntoRulelist(node.leftBranch, antecedent, consequent, ordValue1, ordValue2);
		}
		// Right branch
		else {
			if (node.rightBranch == null)
				node.rightBranch = new RuleNode(antecedent, consequent, ordValue1, ordValue2);
			else
				insertRuleIntoRulelist(node.rightBranch, antecedent, consequent, ordValue1, ordValue2);
		}
	}

	/**
	 * Calculates selector for deciding whether to add rule to left or right branch
	 * of binary-tree rule storage structure.
	 * 
	 * @param node
	 *            the current location in the bin tree.
	 * @param antecedent
	...
	protected boolean insertRuleSelector(RuleNode node, short[] antecedent, double ordValue1, double ordValue2) {
		...
		if (ordValue1 > node.confidenceForRule)
			...
			if (ordValue1 == node.confidenceForRule) {
				...
				// Compare size of antecedent
					...
					if (ordValue2 == node.supportForRule && antecedent.length > node.antecedent.length)
						prcDwnLftBrnch = true;
				}
			}
		}
	...
	/**
	 * Numbers and counts rules contained in binary-tree, usually done when tree is
	 * complete. (CMAR and CBA use a different rule storage structure.)
	 */
	...
	/**
	 * Gets the consequent associated with a particular rule identified by its rule
	 * number.
	 * 
	 * @param ruleNumber
	 *            the identifying number of the desired rule consequent.
	...
	/**
	 * Continues provess of returning consequent associated with a particular rule
	 * identified by its rule number.
	 * 
	 * @param ruleNum
	 *            the identifying number of the desired rule consequent.
	 * @param linkRuleNode
	...
	public short[] getConsequentOfRuleN(int ruleNum, RuleNode linkRuleNode) {
		...
		// Found rule
		if (linkRuleNode.ruleNumber == ruleNum)
			return (linkRuleNode.consequent);
	...
	/**
	 * Copies first N rules contained in one rule bin tree to another new bin tree.
	 * 
	 * @param number
	 *            the number of desired rules.
	 * @param node
	 *            the currentNode.
	...
	protected int copyFirstNrules(int number, RuleNode node) {
		...
		if (node != null) {
			...
			if (number == 0)
				return (number);
			// Node
			insertRuleIntoRulelist(node.antecedent, node.consequent, node.confidenceForRule, node.supportForRule);
			number--;
			if (number == 0)
				return (number);
			// Right branch
	...
	/*
	 * INSERT (ASSOCIATION/CLASSIFICATION) RULE INTO RULE LINKED LIST 2 (ORDERED
	 * ACCORDING SIZE OF ANTECEDENT --- MORE SPECIFIC RULES FIRST).
	 */

	/**
	 * Inserts an (association/classification) rule into the linked list of rules
	 * pointed at by <TT>startRulelist</TT>.
	 * <P>
	 * List is ordered so that more specific rules (i.e. rules with most attributes
	 * in their antecedent) are listed first. **** NOT CURRENTLY USED ****
	 * 
	 * @param antecedent
	 *            the antecedent (LHS) of the rule.
	 * @param consequent
	...
	/*
	 * protected void insertRuleIntoRulelist2(short[] antecedent, short[]
	 * consequent, double confidenceForRule) { // Check for empty tree. if
	 * (startRulelist == null) startRulelist = new RuleNode(antecedent,
	 * consequent,confidenceForRule); // Otherwise "walk" tree else
	 * insertRuleIntoRulelist2(startRulelist,antecedent,consequent,
	 * confidenceForRule); }
	 */
	...
	/*
	 * private void insertRuleIntoRulelist2(RuleNode currentNode, short[]
	 * antecedent, short[] consequent, double orderingValue) { // Left branch if
	 * (antecedent.length>currentNode.antecedent.length) { if
	 * (currentNode.leftBranch==null) currentNode.leftBranch = new
	 * RuleNode(antecedent,consequent,orderingValue); else
	 * insertRuleIntoRulelist2(currentNode.leftBranch,antecedent,
	 * consequent,orderingValue); } // Right branch else { if
	 * (currentNode.rightBranch==null) currentNode.rightBranch = new
	 * RuleNode(antecedent,consequent,orderingValue); else
	 * insertRuleIntoRulelist2(currentNode.rightBranch,antecedent,
	...
	/**
	 * Resizes given item set so that its length is increased by one and new element
	 * inserted.
	 * 
	 * @param oldItemSet
	...
	/**
	 * Resizes given item set so that its length is increased by one and appends new
	 * element (identical to append method)
	 * 
	 * @param oldItemSet
	 *            the original item set
	 * @param newElement
	...
	/**
	 * Resizes given array so that its length is increased by one element and new
	...
	/**
	 * Resizes given array so that its length is decreased by one element and first
	 * element removed
	 * 
	 * @param oldItemSet
	 *            the original item set
	...
	/**
	 * Resize given array so that its length is decreased by size of second array
	 * (which is expected to be a leading subset of the first) and remove second
	 * array.
	 * 
	 * @param oldItemSet
	 *            The first item set.
	 * @param array2
	 *            The leading subset of the <TT>oldItemSet</TT>.
	...
	/**
	 * Merge the two given itemSets to create a new itemSet.
	 * <P>
	 * Note that given itemSets may not be disjoint.
	 * 
	 * @param itemSet1
	 *            The first given item set.
	 * @param itemSet2
	 *            the se4cond given item set.
	...
	/**
	 * Removes leading substring from itemSet1 with respect to its itemSet2 which is
	 * assumed have less elements than itemSet1 and returns the result.
	 * <P>
	 * Example: itemSet1 = [1,2,3], itemSet2 = [1,2], return [3].
	 * 
	 * @param itemSet1
	 *            The first item set.
	 * @param itemSet2
	 *            The leading subset of the <TT>itemSet1</TT>.
	 * @return Revised item set with leading subset removed.
	 */

	/*
	 * protected short[] removeLeadingSubstring(short[] itemSet1, short[] itemSet2)
	 * {
	 * 
	 * 
	 * // Return
	 ...
	 *            the given item set.
	 * @param n
	 *            the index of the element to be removed (first index is 0).
	...
	/**
	 * Removes all elements/attributes after the given element.
	 * <P>
	 * will return null if empty set
	 * 
	 * @param itemSet
	 *            the given item set.
	 * @param n
	 *            the elements after which all element should be removed.
	...
	/**
	 * Removes all elements/attributes whose column number is greater than n
	 * (assumes that item set is ordered high to low).
	 * 
	 * @param oldItemSet
	 *            the given item set.
	 * @param n
	...
	protected short[] returnElementsLessThanN(short[] oldItemSet, int n) {
		...
		if (total == 0)
			...
			for (int index = 0; index < total; index++) {
				if (oldItemSet[index] <= n)
					returnSet[index] = oldItemSet[index];
			}
			return (returnSet);
		}
	}
	...
	/**
	 * Determines relationship between two item sets (same, parent, before, child or
	 * after).
	 * 
	 * @param itemSet1
	 *            the first item set.
	 * @param itemSet2
	 *            the second item set to be compared with first.
	 ...
	 * @param itemSet2
	 *            the second item set to be compared with first.
	 * @return true if itemSet1 is less than or equal (before) itemSet2 and false
	 *         otherwise. Note that before here is not numerical but lexical, i.e.
	 *         {1,2} is before {2}
	 */
	...
	/**
	 * Checks whether a particular element/attribute identified by a column number
	 * is a member of the given item set.
	 * 
	 * @param number
	...
	/**
	 * Checks whether a particular element/attribute identified by a column number
	 * is not a member of the given item set.
	 * 
	 * @param number
	 *            the attribute identifier (column number).
	 * @param itemSet
	...
	/**
	 * Invokes <TT>combinations</TT> method to calculate all possible combinations
	 * of a given item set.
	 * <P>
	 ...
	 * @param sofar
	 *            the part of a combination determined sofar during the recursion
	 *            (null at start).
	 * @param outputSet
	 *            the combinations collected so far, will hold all combinations when
	 *            recursion ends.
	 * @param outputIndex
	 ...
	 *            the given item set.
	 * @param n
	 *            the index up to which the copy is to take place
	 * @return copy of given item set.
	 */
	...
	protected int sizeOfUnion(short[] itemSet1, short[] itemSet2) {
		...
		if (itemSet1 == null) {
			if (itemSet2 == null)
				...
				return (itemSet2.length);
		}
		if (itemSet2 == null)
			return (itemSet1.length);
	 ...
	 * calculation by dinfing the difference between the maximum column entry un the
	 * last column and the last but one column.
	 * 
	...
	/**
	 * Calculates the number of classes by subtracting maximum value in last column
	 * from minimum value in last column.
	 */
	...
	/**
	 * Gets number of attributes/columns in input dataset (note this is not the same
	 * as the number of supported 1 itemsets).
	 * 
	...
	/**
	 * Gets the size (number of attributes) in the first record in the data stored
	 * in the <TT>dataArray</TT> field.
	 * 
	...
	/**
	 * Gets the value of the <TT>numOneItemSets</TT> field, this may either be set
	 * to be equivalent to the number of columns in the input data or represent the
	 * number of "supported" one items sets after the input data has been ordered
	 * and pruned.
	 * 
	...
	/**
	 * Returns the number of generated rules, assumed to be classification rules but
	 * may simply be ARs.
	 * 
	...
	/**
	 * Gets the reference to the start of the (assocoation/classifier) rule list.
	 * 
	...
	/* SET MIN SUPPORT */
	/**
	 * Sets the minimum support (in terms or number of rows in data set) setting.
	 */
	...
	/**
	 * Resets ordered and pruned flags to "false" when fresh input data set is
	 * loaded.
	 */
	...
	/**
	 * Sets the value of the <TT>outputRuleSetToFileFlag</TT> field to the given
	 * value.
	 * 
	 * @param value
	 *            the given value
	 ...
	 * 
	 * @param startRef
	 *            the reference to the start of the rule list.
	 */
	...
	/**
	 * Sets the value for the number of rules field (assumed to be CRs but may be
	 * ARs).
	 */
	 ...
	 * 
	 * @param dataArrayRef
	 *            the reference to the given 2-D data array.
	 */
	 ...
	 * 
	 * @param dataArrayRef
	 *            the reference to the given reconversion array.
	 */
	...
	/**
	 * Outputs stored input data set; initially read from input data file, but may
	 * be reordered or pruned if desired by a particular application.
	 */
	...
	/**
	 * Outputs a given item set to file indicated by <TT>outputFile</TT> field.
	 * 
	 * @param itemSet
	 *            the given item set.
	 */
	...
	/* --------------------------------- */
	/**
	 * Outputs a given item set reconverting it to its original column number labels
	 * (used where input dataset has been reordered and possible pruned). (MAy NO
	 * LONGER BE IN USE!)
	 * 
	 * @param itemSet
	 *            the given item set.
	...
	/* ---------------------- */
	/**
	 * Outputs size (number of records and number of elements) of stored input data
	 * set read from input data file.
	 */
	...
	/**
	 * Outputs conversion array (used to renumber columns for input data in terms of
	 * frequency of single attributes --- reordering will enhance performance for
	 * some ARM algorithms).
	 */
	 ...
	 * Outputs conversion array (used to renumber columns for input data in terms of
	 * frequency of single attributes --- reordering will enhance performance for
	 * some ARM algorithms) (GUI Version.
	 * 
	 * @param textArea
	 *            the given instance of the <TT>JTextArea</TT> class.
	 */
	...
	public void outputConversionArrays(JTextArea textArea) {
		...
		for (int index = 1; index < conversionArray.length; index++) {
			textArea.append("(" + index + ") " + conversionArray[index][0] + ", " + conversionArray[index][1] + "\n");
		}
	 ...
	 * re-conversion if necessary, asuming that the tree represents a set of ARs or
	 * CARS.
	 */
	 ...
	 * 
	 * @param ruleBinTree
	 *            the given rule list (actually a binary tree).
	 */
	...
	/**
	 * Continues process of outputting rules in binary tree as a list of attribute
	 * numbers.
	 * 
	 * @param linkRuleNode
	 *            the currentNode.
	...
	/**
	 * Top level method to output contents of rule binary tree (if any) to the file
	 * indicated by <TT>fileOutput</TT> field, with re-conversion if necessary,
	 * asuming that the list represents a set of ARs or CARS.
	 */
	...
	public void outputRulesToFile() {
		if (startRulelist != null) {
			try {
				...
				JOptionPane.showMessageDialog(null, "Unknown error opening " + "file: " + outputFileName + "\n",
	...
	/**
	 * Continues process of outputting rules in binary tree to the file indicated by
	 * <TT>fileOutput</TT> field as a list of attribute numbers.
	 * 
	 * @param linkRuleNode
	 *            the currentNode.
	 */
	...
	private void outputRulesToFile2(RuleNode linkRuleNode) {
		if (linkRuleNode != null) {
			...
			fileOutput.print(
					"</CONSEQUENT><CONFIDENCE>" + twoDecPlaces(linkRuleNode.confidenceForRule) + "<CONFIDENCE>\n");
			// Right branch
	...
	/**
	 * Outputs contents of rule binary tree (if any) in terns of schema labels,
	 * asuming that the list represents a set of ARs or CARS.
	 */
	...
	/**
	 * Continues process of outputting rules in binary tree as a list using labels
	 * in schema.
	 * 
	 * @param linkRuleNode
	 *            the currentNode.
	...
	/**
	 * Outputs first N rules in rule binary ree (if any) such that the Nth rule is
	 * the default rule.
	 * <P>
	 * Operates by temporeily replacing the value of the <TT>numRules</TT> fild with
	 * the desired number of rules.
	 * 
	 * @param numberOfRulesToBeOutput
	 *            the maximum number of rules to output.
	 */
	 ...
	 * default rule.
	 * <P>
	 * If field <TT>numRules</TT> is temporarily altered this method can be used to
	 * output a particular rule as the default.
	 * 
	 * @param linkRuleNode
	 *            the currentNode.
	 */
	...
	public void outputRulesWithDefault(RuleNode linkRuleNode) {
		...
		if (linkRuleNode != null) {
			// Check if at end
			if (linkRuleNode.ruleNumber > numRules)
				return;
			// Left branch
	...
	public void outputClassifierStats() {
		...
		for (int index = 0; index < antecedentDistrib.length; index++) {
			System.out.println("\tAntecedent size " + (index + 1) + "\t= " + antecedentDistrib[index]);
		}
	}

	/**
	 * Commences process of calculating the maximum antecedent size in the rule set.
	 * 
	...
	/**
	 * Continues process of calculating the maximum antecedent size in the rule set.
	 * 
	 * @param linkRuleNode
	 *            the currentNode.
	 * @param the
	 *            maximum antecedent size discovered so far.
	...
	private int getMaxSizeOfAntecedent(RuleNode linkRuleNode, int maxSofar) {
		...
		if (linkRuleNode != null) {
			...
			// Node
			if (linkRuleNode.antecedent.length > maxSofar)
				maxSofar = linkRuleNode.antecedent.length;
			// Right branch
			maxSofar = getMaxSizeOfAntecedent(linkRuleNode.rightBranch, maxSofar);
		}
	...
	/**
	 * Commences calculation of distribution of antecedent sizes in the rule set.
	 * 
	 * @param antecedentDistrib
	 *            the distribution array in which to store counts for the different
	 *            antecedent sizes.
	 */
	...
	/**
	 * Continues calculation of distribution of antecedent sizes in the rule set.
	 * 
	 * @param linkRuleNode
	 *            the currentNode.
	 * @param antecedentDistrib
	...
	private void calcAntecedentDistrib(RuleNode linkRuleNode, int[] antecedentDistrib) {
		// Process node
		if (linkRuleNode != null) {
			// Left branch
	...
	/**
	 * Calculate and returns the average antecedent size represented in the rule
	 * set.
	 * 
	 * @param antecedentDistrib
	 *            the distribution array in which to store counts for the different
	...
	public void outputDataArray(JTextArea textArea) {
		if (isPrunedFlag)
			textArea.append("DATA SET (Ordered and Pruned)\n" + "-----------------------------\n");
		else {
			if (isOrderedFlag)
				textArea.append("DATA SET (Ordered)\n" + "------------------\n");
			else
				textArea.append("DATA SET\n" + "--------\n");
		}
	...
	public void outputDataArraySchema(JTextArea textArea) {
		if (isPrunedFlag)
			textArea.append("DATA SET (pruned)\n" + "-----------------------------\n");
		else {
			if (isOrderedFlag)
				textArea.append("DATA SET (Ordered)\n" + "------------------\n");
			else
	...
	/**
	 * Outputs contents of rule linked list (if any) in terms of a set of attribute
	 * numbers and asuming that the list represents a set of ARs ( (GUI version).
	 * 
	 * @param textArea
	 *            the text area.
	 */
	...
	private void outputRules2(JTextArea textArea, RuleNode linkRuleNode) {
		if (linkRuleNode != null) {
			...
			textArea.append(" " + twoDecPlaces(linkRuleNode.confidenceForRule) + "\n");
			// Right branch
	...
	/**
	 * Outputs contents of rule linked list (if any) in terms of a set of attribute
	 * numbers and asuming that the list represents a set of CARs and last rue is a
	 * default rule.
	 * 
	 * @param textArea
	 *            the text area.
	...
	/**
	 * Outputs given rule list, with defauklt, in terns of attribute numbers (GUI
	 * version).
	 * 
	 * @param textArea
	 *            the text area.
	...
	/**
	 * Outputs contents of rule linked list (if any) in terns of schema labels
	 * asuming that the list represents a set of ARs (GUI version).
	 * 
	 * @param textArea
	 *            the text area.
	 */
	...
	/**
	 * Outputs given rule list as a set of Prolog facts represented as attribute
	 * numbers.
	 * 
	 * @param textArea
	 *            the text area.
	...
	/**
	 * Continues process of outputting rules in binary tree as a set of Prolog facts
	...
	/**
	 * First calculate and then outputs the minimum support value in terms of number
	...
	public void outputSettings(JTextArea textArea) {
		...
		textArea.append("Min support = " + twoDecPlaces(minSupport) + " (records)\n");
	 ...
	 * 
	 * @param textArea
	 *            the text area to output data to.
	 */

	public void outputSuppAndConf(JTextArea textArea) {
		textArea.append("Support = " + twoDecPlaces(support) + ", Confidence = " + twoDecPlaces(confidence) + "\n");
	}
	...
	public void outputDuration(JTextArea textArea, double time1, double time2) {
		double duration = (time2 - time1) / 1000;
		textArea.append("Generation time = " + twoDecPlaces(duration) + " seconds (" + twoDecPlaces(duration / 60)
				+ " mins)\n");
	}
	...
	/**
	 * Outputs contents of data array to file.
	 * <P>
	 * WARNING will overwrite existing data if stored in the same directory as the
	 * application exacutable, data files are better stored in a separate
	 * "DataFiles" directory.
	 */
	 ...
	 * @param startRecord
	 *            the record marking the start of the segment
	 * @param endRecord
	 *            the record marking the end of the segement.
	 */

	private void ouputSegmentToFile(String outputFileName, int startRecord, int endRecord) throws IOException {
		// Open file for writing
	 ...
	 *            the name of the output file
	 * @param colNumber
	 *            the column number indicating the partition.
	 */

	private void outputPartitionToFile(String fName, short colNumber) throws IOException {

		// Open file for writing
		PrintWriter outputFile = new PrintWriter(new FileWriter(fName + "." + colNumber));
			...
			if (memberOf(colNumber, dataArray[rowIndex])) {
				...
				for (int colIndex = 0; colIndex < dataArray[rowIndex].length; colIndex++) {
					if (firstAtt)
						...
						outputFile.print(" ");
					outputFile.print(dataArray[rowIndex][colIndex]);
					if (dataArray[rowIndex][colIndex] == colNumber)
						break;
				}
				outputFile.println();
			} else
	 ...
	 *            the given item set.
	 * @param outputFile
	 *            the name of the desirted output file.
	 */

	protected void outputItemSetToFile(short[] itemSet, PrintWriter outputFile) throws IOException {
	...
	/**
	 * Compares two real numbers and returns true if the two numbers are the same
	 * within two decimal places.
	 * 
	 * @param number1
	 *            the first given real number.
	 * @param number2
	...
	/**
	 * Converts given real number to real number rounded up to four decimal places.
	 * 
	 * @param number
	 *            the given number.
