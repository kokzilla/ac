 ...
 * record. Note: number of classifiers value is stored in the
 * <TT>numClasses</TT> field in <TT>AssocRuleMining</TT> class.
 * 
 * @author Frans Coenen
...
public class AprioriTFPclass extends PartialSupportTree {
	...
	/**
	 * 2-D array to hold the test data.
	 * <P>
	 * Note that classifiaction involves producing a set of Classification Rules
	 * (CRs) from a training set and then testing the effectiveness of the CRs on a
	 * test set.
	 */
	 ...
	 * <P>
	 * Used in conjunction with "10 Cross Validation" where the input data is
	 * divided into 10 subsets and CRs are produced using each subset in turn and
	 * validated against the remaininmg 9 sets. The oveerall average accuracy is
	 * then the total accuracy divided by 10.
	 */
	...
	/**
	 * Output rule list flag using input schema, true if desired false otherwise.
	 */
	protected boolean ruleListSchemaOutputFlag = false;

	// Other fields
	/**
	 * Number of rows in input data set, not the same as the number of rows in the
	 * classification training set.
	 * <P>
	 * Used for temporery storage of total number of rows when using Ten Cross
	 * Validation (TCV) approach only.
	 * <P>
	 * The <TT>numRows</TT> field inherited from the super class records is used
	 * throughout the CR generation process. Set to number of rows in the training
	 * set using <TT>setNumRowsInInputSet</TT> method called by application class.
	 */
	protected int numRowsInInputSet;
	/**
	 * Number of rows in test set, again not the same as the number of rows in the
	 * classification training set.
	 */
	protected int numRowsInTestSet;
	/**
	 * Number of columns in test set, used for diagnostic purposes only --- should
	 * be same as in training set.
	 */
	protected int numColsInTestSet;
	/**
	 * Number of rows in training set, also not the same as the number of rows in
	 * the classification training set.
	 */
	...
	/**
	 * Constructor with argument from existing instance of class AssocRuleMining.
	 * 
	 * @param armInstance
	 ...
	 * file name, -N = number of classes, -O = output file name, -S = support, -T =
	 * test file name.
	 * <P>
	...
	private void checkNumClasses() {
		if (numClasses == 0) {
			JOptionPane.showMessageDialog(null, "Must specify number of " + "classes (-N)", "COMMAND LINE INPUT ERROR",
					JOptionPane.ERROR_MESSAGE);
			errorFlag = false;
		}
		if (numClasses < 0) {
			JOptionPane.showMessageDialog(null, "Number of classes must be a " + "positive integer",
					"COMMAND LINE INPUT ERROR", JOptionPane.ERROR_MESSAGE);
			errorFlag = false;
		}
	}
	...
	/**
	 * Commences process of loading the test data set (called from application class
	 * when training and test data sets are to be provided separately).
	 */
				...
				if (numColsInTestSet != numCols) {
					JOptionPane
							.showMessageDialog(null,
									"Number of columns " + "in test set is not the\nsame as number of "
	...
	private void readTestFile() {
		try {
			...
			if (inputFormatOkFlag) {
				...
				// Read file
				System.out.println("Reading test set input file: " + testSetFileName);
				readTestDataSet();
			} else
				JOptionPane.showMessageDialog(null, "Unable to read file: " + testSetFileName + "\n",
						"FILE INPUT ERROR", JOptionPane.ERROR_MESSAGE);
	...
	public void readTestDataSet() throws IOException {
		...
		while (line != null) {
			// Process line
			if (!processTestSetLine(line, rowIndex))
				break;
	 ...
	 * @param rowIndex
	 *            the index to the current location in the <TT>testDataArray</TT>
	 *            structure.
	...
	/**
	 * Recasts the contents of the test data array so that each record is ordered
	 * according to conversion array.
	 * <P>
	 * Proceed as follows:
	 * 
	 * 1) For each record in the test data array. Create an empty new itemSet array.
	 ...
	 * itemSet and return into the test data array.
	 */
	...
	public void recastTestData() {
		...
		for (int rowIndex = 0; rowIndex < testDataArray.length; rowIndex++) {
			...
			for (int colIndex = 0; colIndex < testDataArray[rowIndex].length; colIndex++) {
	...
	/**
	 * Reorders input data according to frequency of single attributes but excluding
	 * classifiers which are left unordered at the end of the attribute list.
	 * <P>
	 * Overides method in <TT>AssocRuleMining</TT> class. Note reordering makes for
	 * more efficient executuion of the T-tree (and P-tree) algorithms. Identical
	 * method in <TT>AprioriTclass</TT> class.
	 */
	...
	/**
	 * Removes single attributes (not classifiers) from input data set which do not
	 * meet the minimum support requirement. Identical method in
	...
	public void pruneUnsupportedAtts() {
		...
		for (int rowIndex = 0; rowIndex < dataArray.length; rowIndex++) {
			...
			if (dataArray[rowIndex] != null) {
				...
				// Add classifier
				itemSet = reallocInsert(itemSet, dataArray[rowIndex][maxLength]);
	...
	/**
	 * Adjusts classifier IDs in data array where attributes have been pruned using
	 * <TT>pruneUnsupportedAtts</TT> method.
	 * <P>
	 * Proceeds by looping through data table and subtracting a value equal to the
	 * number of removed attributes from the value of the last element (the
	 * classifier) in each record. Identical method in <TT>AprioriTclass</TT> class.
	 */
	...
	private void recastClassifiers() {
		...
		for (int rowIndex = 0; rowIndex < dataArray.length; rowIndex++) {
			...
			dataArray[rowIndex][lastIndex] = (short) (dataArray[rowIndex][lastIndex] - difference);
		}
		...
		for (int index = startIclassesInRA + numClasses; index < reconversionArray.length; index++) {
	...
	/**
	 * Gets number of supported attributess (note this is not necessarily the same
	 * as the number of columns/attributes in the input set) plus the number of
	 * classifiers.
	 * <P>
	 * Overides parent method which returns the number of support 1 itemsets. This
	 * would exclude any classifiers whose support value was below the minimum
	 * support threshold.
	 * 
	 ...
	 * <P>
	 * Note: (1) assumes a 50:50 split, (2) training data set is stored in the
	 * dataArray structure in which the input data is stored, (3) method called from
	 * application class as same training and test sets may be required if using
	 * (say) "hill climbing" approach to maximise accuracy, (4) method is not called
	 * from constructor partly for same reason as 3 but also because the input data
	 * set may (given a particular application) first require ordering and possibly
	 * also pruning and recasting (see recastClassifiers method).
	 */
	...
	public void createTrainingAndTestDataSets() {
		...
		numRowsInTestSet = (int) ((double) numRows * percentageSizeOfTestSet / 100.0);
	 ...
	 * Populates test and training datasets.
	 * <P>
	 * Note: (1) works on a 9:1 split with nine of the tenths data sets forming the
	 * training set and the remaining one tenth the test set, (2) training data set
	 * is stored in the same dataArray structure in which the initial input data is
	 * stored, (3) this method is not called from the constructor as the input data
	 * set may (given a particular application) first require ordering and possibly
	 * also pruning.
	 * 
	 * @param testSetIndex
	 *            the index of the tenths data sets to be used as the test set.
	 */
	...
	/**
	 * Populates ten tenths data sets for use when doing Ten Cross Validation (TCV)
	 * --- test and training datasets.
	 * <P>
	 * Note: this method is not called from the constructor as the input data set
	 * may (given a particular application) first require ordering (and possibly
	 * also pruning!).
	 */
	...
	public void createTenthsDataSets() {
		...
		// sets
		if (numRows < 10) {
			JOptionPane.showMessageDialog(null, "Only " + numRows + ", therefore cannot create tenths data sets!",
	...
	/**
	 * Commences hill climbing process.
	 * <P>
	 * Finds optimum support and confidence values to provide best classification
	 * accuracy through a hill climbing process. Classification Algorithm uses a 3-D
	 * "hill climbing" method.
	 * 
	 * @param startDsupp
	 *            the start change in support threshold.
	 * @param startDconf
	 *            the start changein confidence threshold.
	 * @param minDsupp
	 *            the minimum change in support threshold.
	 * @param minDconf
	 *            the minimum change in confidence threshold.
	 */

	public void startHillClimbing(double startDsupp, double startDconf, double minDsupp, double minDconf) {
		/* STUBB */
	}
	 ...
	 * <P>
	 * Finds optimum support and confidence values to provide best classification
	 * accuracy through a hill climbing process. Classification Algorithm uses a 2-D
	 * "hill climbing" method (GUI version).
	 * 
	 * @param tArea
	 *            the text area to output data to.
	 * @param startDsupp
	...
	public void startHillClimbing(JTextArea tArea, double startDsupp, double startDconf, double minDsupp,
			double minDconf) {
		/* STUBB */
	}
	 ...
	 * <P>
	 * Identical method in <TT>AprioriTclass</TT>.
	 */
	...
	/**
	 * Searches through rule data looking for a rule antecedent which is a subset of
	 * the input set or the default rule (last rule).
	 * <P>
	 * Copy of method in AprioriTclass.
	 * 
	 * @param itemSet
	...
	protected short classifyRecordDefault(short[] itemSet, RuleNode node) {
		...
		if (node != null) {
			...
			if (consClass != 0)
				return (consClass);
			// Node
			if ((node.ruleNumber == numRules) || (isSubset(node.antecedent, itemSet)))
				return (node.consequent[0]);
			// Right branch
	...
	/**
	 * Claculate the MWW (Man-Whitney-Wilcoxon) statistic for the given class pair.
	 * 
	 * @param i
	 *            response MWW table column class identifier.
	 * @param j
	 *            signal MWW table columnclass identifier.
	 * @param responseTable
	 *            the response table.
	 * @param signalTable
	 *            the signal table.
	...
	protected double calcMWWstatValue(int class_i, int class_j, int[][] responseTable, int[][] signalTable) {
		...
		if (n1 == 0 || n2 == 0)
			...
			mwwValue = ((double) totalRanking - (((double) n1 * (n1 + 1)) / 2.0)) / (double) (n1 * n2);
		return (mwwValue);
	}
	...
	private void rankMWWtable(int[][] mwwTable) {
		...
		do {
			...
			while (index < (mwwTable.length - 1)) {
				if (isBeforeInMWWtable(mwwTable[index][0], mwwTable[index][1], mwwTable[index + 1][0],
						mwwTable[index + 1][1]))
					index++;
				else {
					isOrdered = false;
	 ...
	 * 
	 * Schema: Response1, Signal1, Response2, Signal2. {0,1,0,1} equal {0,1,0,0}
	 * before {0,1,1,0} before {0,1,1,1} before {0,0,0,1} after (swop) {0,0,0,0}
	 * equal {0,0,1,0} before {0,0,1,1} before {1,0,0,1} after (swop) {1,0,0,0}
	 * after (swop) {1,0,1,0} equal {1,0,1,1} before {1,1,0,1} after (swop)
	 * {1,1,0,0} after (swop) {1,1,1,0} after (swop) {1,1,1,1} equal
	 * 
	 * @param response1
	 *            response value (1 or 0) for record 1.
	 * @param signal1
	 *            signal value (1 or 0) for record 1.
	 * @param response2
	 *            response value (1 or 0) for record 2.
	 * @param signal2
	 *            signal value (1 or 0) for record 2.
	 * @return true if record 1 is before record 2, and false otherwise.
	 */

	private boolean isBeforeInMWWtable(int response1, int signal1, int response2, int signal2) {
	...
	/**
	 * Determine if the testing of generated classifier can be undertaken, i.e. is
	 * there a test set? and is there a classifier? private boolean true if OK,
	 * false otherwise.
	 */
	...
	/**
	 * Tests the generated classification rules using test set with chi^2 and best K
	 * strategy, and return percentage accuracy.
	 * <P>
	 * Rules ordered according to chi^2 value and clasas selected by considering the
	 * average result of firing K rules.
	 * 
	...
	protected double testClassificationBestK() {
		...
		int index = 0;
		for (; index < testDataArray.length; index++) {
			short classResult = classifyRecordBestKaverage(classifiers, testDataArray[index]);
			if (classResult != 0) {
				short classActual = getLastElement(testDataArray[index]);
				if (classResult == classActual)
					correctClassCounter++;
	 ...
	 * accuracy.
	 * <P>
	 * Operates as follows: 1) Obtain all rules whose antecedent is a subset of the
	 * given record. 2) Select the best K rules for each class (rule with highest
	 * ranking). 3) Determine the average expected accuracy over the selected rules
	 * for each class, 4) Select the class with the best average expected accuracy.
	 * 
	 * @param classification
	 *            the possible classers.
	 * @param itemSet
	 *            the record to be classified.
	 * @return the class label.
	 */

	protected short classifyRecordBestKaverage(short[] classification, short[] itemSet) {
		...
		short classLabel = (short) selectClassWithBestAverage(averages, classification[0]);
	...
	/**
	 * Places all rules that satisfy the given record in a rule linked list pointed
	 * at by startRulelist field, in the order that rules are presented.
	 * <P>
	 * Used in Best K Average. Identical method in <TT>Classification</TT> class.
	 * 
	 * @param itemset
	 *            the record to be classified.
	 * @param linkref
	 *            The reference to the start of the existing list of rules.
	 */
	...
	private void obtainAllRulesForRecord(short[] itemSet, RuleNode linkRef) {
		...
		if (linkRef != null) {
			...
			if (isSubset(linkRef.antecedent, itemSet)) {
				insertRuleIntoRulelist(linkRef.antecedent, linkRef.consequent, linkRef.confidenceForRule,
	...
	/**
	 * Keep only best K best rules for each class by processing selected rules list.
	 * <P>
	 * Bug reports: Thanks to Jian (Ella) Chen, Dept Comp. Sci. School of
	 * Information Sci. and Tech. Sun Yat-sen University, China. Bin tree is process
	 * and a new bin tree created containing the selected rules. Note also that
	 * rules are stored according to some ordering. Identical method in
	 * <TT>Classification</TT> class.
	 * 
	 * @param kValue
	 *            the maximum number of rules to be kept per class.
	 * @param classification
	...
	/**
	 * Identifies first K rules in bin tree with given consequent and places them in
	 * the rule bin tree the start of which is pointed at by the
	 * <TT>startRulelist</TT> field.
	 * <P>
	...
	private int getFirstKrulesInBinTree(int kValue, short consClass, RuleNode node) {
		if (node != null) {
			// Left branch
	 ...
	 * <P>
	 * Accuracy here is measured in terms of confidence or some other measure stored
	 * in the "confidenceForRule" field. Identical method in <TT>Classification</TT>
	 * class.
	 * 
	 * @param decrement
	 *            the label for the first class in the classification array which is
	 *            used here to relate class labels to indexes.
	...
	private double[] getAverageAccuracies(int decrement) {
		...
		for (int index = 0; index < accuracies.length; index++) {
			if (results[0][index] != 0)
				accuracies[index] = results[0][index] / results[1][index];
		}
	 ...
	 * 
	 * @param results
	 *            the results array describing accuracies and totals for each class.
	 * @param decrement
	 *            the label for the first class in the classification array which is
	 *            used here to relate class labels to indexes.
	 * @param node
	...
	private void getAverageAccuracies(double[][] results, int decrement, RuleNode node) {
		if (node != null) {
			// Left branch
	...
	/* SELECT CLASS WITH BEST AVERAGE */
	/**
	 * Returns the class label with the best average accuracy associated with it.
	 * <P>
	 * Identical method in <TT>Classification</TT> class.
	 * 
	 * @param increment
	...
	/**
	 * Assigns value of <TT>numTows</TT> field to the <TT>numRowsInInputSet</TT>
	 * field.
	 * <P>
	 * Used in conjunction with TCV to "remember" the overall number of rows in the
	 * input data set.
	 * <P>
	...
	/**
	 * Assigns a value equavalent to the number of rows to the number of rows in
	 * training set field.
	 * <P>
	 * used when the entire data set is considered as the training set.
	 * 
	 * @param numTSrows
	 *            given number of rows in training set.
	 */
	 ...
	 * 
	 * @param newTestDataArray
	 *            the given 2-D array of terst data attributes.
	 */
	 ...
	 * <P>
	 * Different to <TT>getNumCRS</TT> method in <TT>AssocRuleMining</TT> class
	 * which actually calculates the number of rules.
	 * 
	...
	/**
	 * Outputs command line values provided by user. (Overides higher level method.)
	 */
	...
	protected void outputSettings() {
		...
		if (testSetFileName != null)
			System.out.println("Test set file name " + "(optional) = " + testSetFileName);
		if (outputFileName != null)
			System.out.println("Output file name " + "(optional)   = " + outputFileName);
	...
	protected void outputLimits() {
		System.out.println("Max num frequent sets  = " + MAX_NUM_FREQUENT_SETS);
		System.out.println("Max size of antecedent = " + MAX_SIZE_OF_ANTECEDENT);
	 ...
	 * 
	 * @param parameters
	 *            the 2D array contaning details of results.
	 */
	...
	protected void tcvOutput(double[][] parameters) {
		...
		// Output avergaes
		outputTCVaverages(averageAccuracy, sdAccuracy, averageAUCvalue, averageNumFreqSets, averageNumUpdates,
				averageNumCRs);
	}
	 ...
	 * 
	 * @param avAcc
	 *            the average accuracy as the result of TCV.
	 * @param sdAccuracy
	 *            the standard deviation from average accuracy.
	 * @param aveAUC
	 *            the average AUC calue as a result of TCV.
	 * @param aveNumFreqSets
	 *            the average number of frequent sets as the result of TCV.
	 * @param aveNumUpdates
	 *            the number of updates as the result of TCV.
	 * @param aveNumCRs
	 *            the average number of callsification rules as the result of TCV.
	 */

	private void outputTCVaverages(double aveAcc, double sdAccuracy, double aveAUC, double aveNumFreqSets,
			double aveNumUpdates, double aveNumCRs) {
		// Generate string
		String s = "---------------------------------------\n" + "Average Accuracy\t= " + twoDecPlaces(aveAcc)
				+ "\nSD Accuracy\t\t= " + twoDecPlaces(sdAccuracy) + "\nAverage AUC value = "
				+ fourDecPlaces(averageAUCvalue) + "\nAve. # Freq. Sets\t= " + twoDecPlaces(aveNumFreqSets)
				+ "\nAvergae Num Updates\t= " + twoDecPlaces(aveNumUpdates) + "\nAverage Num CRs\t= "
				+ twoDecPlaces(aveNumCRs) + "\n---------------------------------------";
	...
	private void ouputTCVparam(double[][] parameters) {
		...
		for (int index = 0; index < parameters.length; index++) {
			String s = "(" + (index + 1) + ") Accuracy = " + twoDecPlaces(parameters[index][0]) + ", AUC value = "
					+ fourDecPlaces(parameters[index][1]) + ", \t# Freq. Sets = " + ((int) parameters[index][2])
					+ ", \t# Updates = " + ((int) parameters[index][3]) + ", \t# CRs = " + ((int) parameters[index][4])
					+ "\n";
			// Output
			if (textArea == null)
				System.out.println(s);
