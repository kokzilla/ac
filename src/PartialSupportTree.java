...
public class PartialSupportTree extends TotalSupportTree {
	...
	/**
	 * Structurte to contain P-tree data in tabular form for improved computational
	 * efficiency when creating T-tree.
	 * <P>
	 * A 2-D array of these structures is created in which to store the Ptree.
	 */
	...
	protected class PtreeRecord {
		...
		/**
		 * Union of a pTree node label (<TT>pTreeNodeLabel</TT>) and all its ancestor
		 * node labels.
		 */
		 ...
		 * 
		 * @param nodeLabel
		 *            the label for P-tree node.
		 * @param itemSet
		 *            the uunion of a pTree node label (<TT>nodeLabel</TT>) and all its
		 *            ancestor node labels.
		 * @param sup
		 *            the partial support count.
		 */
	...
	/**
	 * Array of arrays data structures for P-tree table (used as a computational
	 * efficiency measure).
	 */
	protected PtreeRecord[][] startPtreeTable = null;
	/**
	 * Array for holding the number of P-tree nodes for each possible cardinality
	 * given the number of attributes, maximum is equal to number of columns.
	 */
	protected int[] pTreeNodesOfCardinalityN = null;
	/**
	 * Array of "markers" used during the generation of the P-tree, and contains
	 * "current index" values for each level of cardinality.
	 */
	...
	/**
	 * Flag indicating whether it is OK to delete the P-tree branch by branch as the
	 * P-tree table is built (true by default).
	 */
	...
	public void createPtree() {
		...
		for (int index = 0; index < dataArray.length; index++) {
			if (dataArray[index] != null)
				addToPtreeTopLevel(dataArray[index]);
		}

		// Potential output
		if (outputPtreeStatsFlag)
			outputPtreeStats();
		if (outputPtreeFlag)
			outputPtree();
	...
	public void createPtree(JTextArea textArea) {
		...
		// Potential output
		if (outputPtreeStatsFlag)
			outputPtreeStats(textArea);
		if (outputPtreeFlag)
			outputPtree(textArea);
		// if (outputPtreeGraphFlag) drawPtreeGraph();
	...
	/**
	 * Commences process to add an itemset to the P-tree starting with the top level
	 * of the tree.
	 * <P>
	 * Note that the top level is an array.
	 * 
	 * @param itemSet
	 *            the given item set.
	 */
	...
	protected void addToPtreeTopLevel(short[] itemSet) {
		...
		if (itemSetLength == 1) { // Top level
			if (startPtreeRef[index] == null) {
				...
				startPtreeRef[index].support = startPtreeRef[index].support + 1;
			numPtreeNodeUpdates++;
		}
	 ...
	 * <LI>code = 5, Itemset after and not superset of current node (younger
	 * sibling)
	 * </UL>
	 * <P>
	 ...
	 * 
	 * @param flag
	 *            the type of branch currently under consideration: code 0 = root, 1
	 *            = child, 2 = sibling.
	 * @param parentLength
	 *            the number of elements represented by the parent node of the
	 *            current node. Used only when adding new "dummy" nodes to maintain
	 *            count of number of nodes of a given size for when the Ptree table
	 *            is generated.
	 * @param itemSetLength
	 *            the number of elements in the current itemSet. Used only when
	 *            adding new nodes to maintain count of number of nodes of a given
	 *            size for when the Ptree table is generated.
	 * @param linkRef
	 *            the reference (pointer) to current location in the P-tree.
	 * @param itemSet
	 *            the row itemSet in the input file currently under consideration.
	 * @param topIndex
	 *            the index of the element in the array marking the top level of the
	 *            P-tree, used only when inserting new nodes hanging from this top
	...
	protected void addToPtree(int flag, int parentLength, int itemSetLength, PtreeNode linkRef, short[] itemSet,
		...
		// Otherwise process tree
			...
			case 2: /* Rule 2: Before and subset (parent) */
				beforeAndSubset(flag, itemSetLength, linkRef, itemSet, topIndex, oldRef);
				break;
			case 3: /* Rule 3: Before and not subset (elder sibling) */
				beforeAndNotSubset(flag, parentLength, itemSetLength, linkRef, itemSet, topIndex, oldRef);
				break;
			case 4: /* Rule 4: After and superset (child) */
				afterAndSuperset(parentLength, itemSetLength, linkRef, itemSet);
				break;
			case 5: /* Rule 5: After and not superset (younger sibling) */
				afterAndNotSuperset(flag, parentLength, itemSetLength, linkRef, itemSet, topIndex, oldRef);
				break;
	...
	/**
	 * Adds new node into the P-tree on a parent/child link so that the new node is
	 * the parent of the existing child branch and the child of the previous
	 * "parent"; also checks if any siblings need to be "moved up".
	 * <P>
	 * Possibilities:
	 * <OL>
	 ...
	 * <LI>Connect to child ref with siblings moved up ({1 2 3 4} {1 2 4 5} {1 2} {1
	 * 2 3})
	 * <LI>connect to sibling ref with no siblings moved up ({1 2} {1 3 4} {1 3})
	 * <LI>connect to sibling ref with siblings moved up ({1 2} {1 3 4} (1 4) {1 3})
	 * </OL>
	 * 
	 * @param flag
	 *            the type of branch currently under consideration: code 0 = root, 1
	...
	private void beforeAndSubset(int flag, int itemSetLength, PtreeNode linkRef, short[] currentitemSet, int topIndex,
			PtreeNode oldRef) {
	 ...
	 * "current" node.
	 * <P>
	 * First checks for leading substring with existing node; if found and leading
	 * substring is not same as current parent creates a new P-tree node for this
	 * substring and then adds in the new node. Possibilities:
	 * <OL>
	 * <LI>Connect to top level node with no common leading substring with current
	 * node ({1 3} {1 2}).
	 * <LI>Connect to top level node with common leading substring with current node
	 * and no siblings moved up ({1 2 4} {1 2 3}).
	 * <LI>Connect to top level node with common leading substring with current node
	 * and siblings moved up ({1 2 4} (1 3} {1 2 3}).
	 * <LI>Connect to child ref with no common leading substring with current node
	 * ({1 2} {1 2 4} {1 2 3}).
	 * <LI>Connect to child ref with common leading substring with current node and
	 * no siblings moved up ({1 2} {1 2 4 5 7} {1 2 4 5 6}).
	 * <LI>Connect to child ref with common leading substring with current node and
	 * siblings moved up ({1 2} {1 2 4 5 7} {1 3} {1 2 4 5 6}).
	 * <LI>Connect to sibling ref with no common leading substring with current node
	 * ({1 2} {1 4} {1 3}).
	...
	private void beforeAndNotSubset(int flag, int parentLength, int itemSetLength, PtreeNode linkRef,
		...
		if (subsetItemSet != null) { // Leading substring exists
			...
			PtreeNode newParentRef = createPtreeNode(subsetItemSet, subsetItemSet.length + parentLength - 1);
			...
			newParentRef.childRef = createPtreeNode(currentItemSet, itemSetLength);
			...
			// Create new node
			PtreeNode newSiblingRef = createPtreeNode(currentItemSet, itemSetLength);
	...
	/**
	 * Insets node into P-tree where new itemset is an child of the "current" node.
	 * <P>
	 * If no more child nodes add new node to "current" node as child, else carry on
	 * down the tree with flag set to 1 to indicate we are following a child branch.
	 * Possibilities:
	 * <OL>
	...
	private void afterAndSuperset(int parentLength, int itemSetLength, PtreeNode linkRef, short[] currentItemSet) {
		...
		if (linkRef.childRef == null) {
			...
			PtreeNode newRef = createPtreeNode(realloc4(currentItemSet, linkRef.itemSet), itemSetLength);
		...
		// continue down child branch
			...
			addToPtree(1, parentLength + linkRef.itemSet.length, itemSetLength, linkRef.childRef,
					realloc4(currentItemSet, linkRef.itemSet), 0, linkRef);
	}

	/* AFTER AND NOT SUPERSET */

	/**
	 * Commeences process of inserting node into P-tree where new itemset is a
	 * younger sibling of the "current" node.
	 * <P>
	 * Possible actions:
	 * <OL>
	...
	private void afterAndNotSuperset(int flag, int parentLength, int itemSetLength, PtreeNode linkRef,
			short[] currentItemSet, int topIndex, PtreeNode oldRef) {

		// Test if end of sibling branch, if not continue

		if (linkRef.siblingRef == null)
			afterAndNotSuperset1(flag, parentLength, itemSetLength, linkRef, currentItemSet, topIndex, oldRef);

		// Not end of sibling branch

		else
			afterAndNotSuperset2(flag, parentLength, itemSetLength, linkRef, currentItemSet, topIndex, oldRef);
	}
	 ...
	 * <P>
	 * Also tests for leading substring. If found and this is not equal to an
	 * existing parent itemSet method causes a dummy node to represent the substring
	 * to be inserted (call to <TT>addSupport2</TT>). Possibilities:
	 * <OL>
	 * <LI>Add to siblingRef with no common leading substring ({1 2} {1 3}).
	 * <LI>Add to siblingRef with common leading substring ({1 2} {1 3 5} {1 3 4}).
	 * </OL>
	 * 
	 * @param flag
	 *            the type of branch currently under consideration: code 0 = root, 1
	...
	private void afterAndNotSuperset1(int flag, int parentLength, int itemSetLength, PtreeNode linkRef,
			short[] currentItemSet, int topIndex, PtreeNode oldRef) {
		...
		if (subsetItemSet != null) {
			// Create new parent representing subset
			PtreeNode newParent = createPtreeNode(subsetItemSet, subsetItemSet.length + parentLength - 1);
			// Add support made up of existing support + 1 for current row
			...
			linkRef.siblingRef = createPtreeNode(realloc4(currentItemSet, subsetItemSet), itemSetLength);
		}
	...
	/**
	 * Inserts node into P-tree where new itemset is a younger sibling of the
	 * "current" node and there are more younger sibling on current existing node.
	 * <P>
	 * Possible actions:
	 * <OL>
	 * <LI>If there are more sibling nodes and the current row itemSet shares a
	 * leading substring with the current P-tree node and this is not equal to an
	 * existing parent itemSet then add a dummy node to represent the substring
	 * (call <TT>addSupport2</TT>). Then add new node.
	...
	private void afterAndNotSuperset2(int flag, int parentLength, int itemSetLength, PtreeNode linkRef,
			short[] currentItemSet, int topIndex, PtreeNode oldRef) {
		...
		// Otherwise carry on along sibling branch
			...
			addToPtree(2, parentLength, itemSetLength, linkRef.siblingRef, currentItemSet, 0, linkRef);
	}
	 ...
	 *            = child, 2 = sibling.
	 * @param newRef
	 *            the reference (pointer) to newly created parent indicating current
	 *            laction in the P-tree.
	 * @param topIndex
	 *            the index of the element in the array marking the top level of the
	 *            P-tree, used only when inserting new nodes hanging from this top
	 *            level otherwise ignored.
	...
	/**
	 * Checks sibling branch to determine whether the siblings are all supersets of
	 * the parent and readjusts P-tree accordingly.
	 * <P>
	 * Possibilities:
	 * <OL>
	 * <LI>Sibling branch is empty (do nothing).
	 * <LI>No nodes in sibling branch are supersets of parent there for move them
	 * all up to be siblings of the parent
	 * <LI>All nodes in sibling branch are supersets of parent, thus do nothing.
	 * <LI>Some nodes in sibling branch are supersets of parent, others are not;
	 * therefore move those that are not up to be siblings of the parent.
	 * </OL>
	 * Note: when a node is found that is not a superset of the parent we do not
	 * need to keep on checking.
	 * 
	 * @param linkRef
	 *            the reference (pointer) to the current node.
	 * @param parentItemSet
	 *            the itemset label represented by the parent.
	 * @param newRef
	 *            tghe reference (pointer) to the newly created parent node.
	 */
	 ...
	 * 
	 * @param itemSet
	 *            the itemset to be stored at the node.
	 * @param level
	 *            the cardinality (length) of the item set to be stored in the node.
	 */
	 ...
	 * 
	 * @param linkPtreeRef
	 *            the reference/pointer to the current location in the P-tree.
	 * @param totalpTreeItemSet
	 *            the union of all the parent labels sofar.
	 * @param currentLevel
	 *            the current levl in the P-tree, initially set to 1.
	 */
	 ...
	 * 
	 * @param pTreeNodeLabel
	 *            the union of all the parent labels.
	 * @param pTreeItemSet
	 *            the node label.
	 * @param support
	 *            the support associated with the node
	 * @param level
	 *            the current levl in the P-tree.
	 */
	...
	private void addToPtreeArray(short[] pTreeNodeLabel, short[] pTreeItemSet, int support, int level) {
		...
		if (pTreeNodeLabel == null) {
			...
			startPtreeTable[level][pTreeTableMarker[level]] = new PtreeRecord(pTreeNodeLabel,
					append(pTreeItemSet, pTreeNodeLabel), support);
	...
	/**
	 * Commences process of generating a total support tree (T-tree) by initilising
	 * data structures.
	 * <P>
	 * Overides method in TotalSupportTree class, distinction is that this method
	 * causes input data to be read from a P-tree as opposed to a data file.
	 */

	public void createTotalSupportTree() {
		System.out.println("Apriori-TFP with X-Checking\nMinimum support " + "threshold = " + twoDecPlaces(support)
	...
	/**
	 * Commences process of generating a total support tree (T-tree): GUI version.
	 * 
	 * @param textArea
	 *            the text area.
	 */
	...
	/*
	 * Set of methods for creating T-tree from P-tree which overide methods of smae
	 ...
	 * through table level by level (row by row).
	 * <P>
	 * Overides method in TotalSupportTree class (operates using a P-tree tavle
	 ...
	 * 
	 * @param pTreeTableLevel
	 *            the given level (row) of P-tree table records.
	 */
	...
	/**
	 * Increments support counts in T-tree top level given a P-tree table label and
	 * an associated support value.
	 * 
	 * @param pTreeNodeLabel
	 *            the label associated with a P-tree node (not the union of its
	 *            parent labels).
	 * @param pTreeNodeSupport
	...
	private void createTtreeTopLevel4(short[] pTreeNodeLabel, int pTreeNodeSupport) {
		...
		for (int index = 0; index < pTreeNodeLabel.length; index++) {
			...
			startTtreeRef[pTreeNodeLabel[index]].support = startTtreeRef[pTreeNodeLabel[index]].support
					+ pTreeNodeSupport;
			numUpdates++;
		}
	}
	...
	/**
	 * Commences process of adding support values to further levels of the T-tree
	 * (not the top level) by stepping through the Ptree table from the current
	 * required level up to the maximum level that may be contained in the table.
	 * 
	 * @param level
	 *            the (start) current level.
	 */
	...
	protected void addSupportToTtreeLevelN(int level) {
		...
		for (int index1 = level; index1 < startPtreeTable.length; index1++) {
			...
			if (startPtreeTable[index1] != null) {
				...
				for (int index2 = 0; index2 < startPtreeTable[index1].length; index2++) {
					addSupportToTtreeLevelN(level, startPtreeTable[index1][index2].pTreeNodeLabel,
							startPtreeTable[index1][index2].pTreeItemSet, startPtreeTable[index1][index2].support, 1,
							startTtreeRef.length);
				}
			}
		}
	...
	/**
	 * Continues process of incrementing support values for appropriate nodes in
	 * T-tree at a given level and given a record from the P-tree table.
	 * <P>
	 * This method identifies the top level T-tree nodes whose child branch we need
	 * to follow. We only need to process those branches contained in the P-tree
	 * node label (not the complete itemset represented by the node, i.e. the union
	 * of the node label and all its parent labels). Method includes bounds on which
	 * branches should be processed: on start up the lower bound is attribute 1 as
	 * this attribute cannot have a sub-branch, the upper bound is the last
	 * attribute plus 1 in the top level array of T-tree nodes in the T-tree.
	 * 
	 * @param level
	 *            the desired level in T-tree
	 * @param pTreeNodeLabel
	 *            the actual P-tree node itemSet label (not the union of its parent
	 *            labels).
	 * @param pTreeItemSet
	 *            the Union of the pTreeNodeLabel and all parent node itemSets of
	 *            the current node.
	 * @param support
	 *            the upport count.
	 * @param startAttribute
	 *            the attribute marking the start of the range of attributes in the
	 *            pTreeNodeLabel to be processed further.
	 * @param endAttribute
	 *            the attribute marking the end of the range of attributes in the
	 *            pTreeNodeLabel to be processed further.
	 */

	protected void addSupportToTtreeLevelN(int level, short[] pTreeNodeLabel, short[] pTreeItemSet, int support,
			int startAttribute, int endAttribute) {
		// Loop through node label
		for (int index = 0; index < pTreeNodeLabel.length; index++) {
			if (pTreeNodeLabel[index] > startAttribute && pTreeNodeLabel[index] < endAttribute
				...
				if (currentNode.childRef != null)
					addSupportToTtreeLevelN(currentNode.childRef, level - 1, pTreeItemSet, support);
			}
		}
	}
	...
	/**
	 * Processes a branch in the T-tree with the aim of incrementing support values
	 * for appropriate nodes in T-tree at a given level in the branch and for a
	 * given a record from the P-tree table.
	 * 
	 * @param linkRef
	 *            the reference (pointer) to the current branch in the T-tree (top
	 *            at start)
	 * @param level
	 *            the desired level in T-tree
	 * @param pTreeItemSet
	...
	protected void addSupportToTtreeLevelN(TtreeNode[] linkRef, int level, short[] pTreeItemSet, int support) {
		// At right level
		if (level == 1)
			addSupToArrayOfTtreeNodes(linkRef, pTreeItemSet, support);

		// At wrong level
		else
			addSupportToTtreeLevelN2(linkRef, level, pTreeItemSet, support);
	}

	/**
	 * Continues the process of incrementing support values T-tree where the desired
	 * kevel in the T-tree has not yet been found.
	 * 
	 * @param linkRef
	 *            the reference (pointer) to the current branch in the T-tree (top
	 *            at start)
	 ...
	 *            the current node.
	 * @param support
	 *            the Ptree node support count.
	 */

	protected void addSupportToTtreeLevelN2(TtreeNode[] linkRef, int level, short[] pTreeItemSet, int support) {
		...
		for (int index = 0; index < pTreeItemSet.length; index++) {
			...
			if (pTreeItemSet[index] >= linkRef.length)
				break;
	 ...
	 * 
	 * @param linkRef
	 *            the reference (pointer) to the given array of T-tree nodes.
	 * @param pTreeItemSet
	 *            the Union of the pTreeNodeLabel and all parent node itemSets of
	 *            the current node.
	 * @param support
	...
	protected void addSupToArrayOfTtreeNodes(TtreeNode[] linkRef, short[] pTreeItemSet, int support) {
	...
	/**
	 * Commences the process of Finding the support of an itemset in the P-tree.
	 * 
	 * @param itemSet
	 *            the given itemset.
	 */
	...
	public int findInPtree(short[] itemSet) {
		...
		for (int index = 1; index < itemSet[0] && index < startPtreeRef.length; index++) {
			if (startPtreeRef[index] != null)
				support = support + findInPtree(startPtreeRef[index].childRef, itemSet);
		}
		...
		if (itemSet[0] < startPtreeRef.length && startPtreeRef[itemSet[0]] != null) {
			...
			if (itemSet.length == 1)
				support = support + startPtreeRef[itemSet[0]].support;
			// Otherwise proceed down branch
			else
				support = support + findInPtree(startPtreeRef[itemSet[0]].childRef, realloc3(itemSet));
		}
	 ...
	 * Possibilities: 1) Empty P-tree node: return 0. 2) Equality: Itemset is
	 * identical to node label, return support. 3) Before: First element in itemset
	 * lexically before fiest item in node label return 0; 4) Subset: Itemset is
	 * subset of node label, store support and check sibling. 5) Superset: Itemset
	 * is superset of node label, remove leading sub-string and proceed down child
	 * branch. 6) After: Otherwise proceed down sibling and child branches.
	 */
	...
	private int findInPtree(PtreeNode linkRef, short[] itemSet) {
		...
		if (linkRef == null)
			return (0);
		// Equality
		if (isEqual(itemSet, linkRef.itemSet))
			return (linkRef.support);
		// Before
		else if (itemSet[0] < linkRef.itemSet[0])
			return (0);
		// Subset
		else if (isSubset(itemSet, linkRef.itemSet)) {
			if (linkRef.siblingRef != null)
				return (linkRef.support + findInPtree(linkRef.siblingRef, itemSet));
			else
				return (linkRef.support);
		}
		// Superset (subset test but the otherway round)
			...
			if (linkRef.childRef != null)
				return (findInPtree(linkRef.childRef, realloc4(itemSet, linkRef.itemSet)));
			else
				return (0);
		}
		// After
			...
			int tempSupport = 0;
			if (linkRef.siblingRef != null)
				tempSupport = findInPtree(linkRef.siblingRef, itemSet);
			if (linkRef.childRef != null)
				tempSupport = tempSupport + findInPtree(linkRef.childRef, itemSet);
			return (tempSupport);
		}
	}
	...
	/**
	 * Sets flag indicating whether it is ok (true) ot not OK (false) to delete the
	 * P-tree branch by branch as the P-tree table is generated. Typically not OK if
	 * in incremental mode where we may wish to return to the P-tree.
	 * 
	 * @param value
	 *            the value for the flag.
	 */
	 ...
	 * 
	 * 1) Output P-tree 2) Output P-tree GUI version 3) Output P-tree Table
	 * Statistics (ordinary and GUI version) 4) Output P-tree Table 5) Output P-tree
	 * Table statistics 6) Output P-tree Graph.
	 */
	 ...
	 * 
	 * @param linkPtreeRef
	 *            the reference to the start of the P-tree.
	 */
	 ...
	 * 
	 * @param index
	 *            the current index in the top-level (array) of tghe P-tree.
	 * @param linkRef
	 *            the reference to the P-tree top level node in question.
	 * @param counter
	 *            the node counter (not necesserily the same as the index if some
	 *            nodes are absent).
	 */
	 ...
	 * 
	 * @param linkRef
	 *            the reference to the current location in the P-tree.
	 * @param node
	 *            the identifier for the current node (for output purposes only).
	 * @param counter
	 *            the node counter (used to generate a new node identifier).
	 */
	...
	private void outputPtree2(JTextArea textArea, int index, PtreeNodeTop linkRef, int counter) {
		String newNode = Integer.toString(counter);
	...
	private void outputPtree3(JTextArea textArea, PtreeNode linkRef, String node, int counter) {
		String newNode;
	 ...
	 * purposes): (a) Storage, (b) Number of nodes on P-tree, (c) number of partial
	 * support increments (updates) and (d) generation time.
	 */
	 ...
	 * 
	 * @param textArea
	 *            the given instance of thge calss JtextArea.
	 */

	public void outputPtreeStats(JTextArea textArea) {
		textArea.append("P-TREE STATISTICS\n-----------------------------\n");
		textArea.append(calculateStorage(startPtreeRef) + " (Bytes) storage\n");
	 ...
	 * 
	 * @param linkPtreeRef
	 *            the reference to the current portion of the P-tree.
	...
	/**
	 * Commences process to calculates storage requirements for a branch of the
	 * P-tree eminating from the top level.
	 * 
	 * @param storage
	 *            the required storage sofar.
	 * @param linkref
	 ...
	 *            the required storage sofar.
	 * @param linkref
	 *            the reference to current location in the P-tree branch.
	...
	/**
	 * Outputs total number of P-tree nodes (and the number of support value
	 * increments).
	 */
	...
	public void outputNumNodes() {
		...
		System.out.println("Number of P-tree updatess = " + numPtreeNodeUpdates);
	}
	...
	/**
	 * Commence process of determining total number of nodes in (sub-) branch of
	 * P-tree.
	 * 
	 * @param linkPtreeRef
	 *            the reference to the current location in the P-tree.
		...
		for (int index1 = 1; index1 < startPtreeTable.length; index1++) {
			...
			if (startPtreeTable[index1] == null)
				...
				for (int index2 = 0; index2 < pTreeTableMarker[index1]; index2++) {
					...
					outputItemSet(textArea, startPtreeTable[index1][index2].pTreeNodeLabel);
					textArea.append(" Itemset = ");
					outputItemSet(textArea, startPtreeTable[index1][index2].pTreeItemSet);
					textArea.append(" sup = " + startPtreeTable[index1][index2].support + "\n");
				}
			}
		}
	}
	...
	/*
	 * public void drawPtreeGraph() { // If more than 1000 frequent sets return if
	 * (getNumPtreeNodes()>maxPtreeGraphNodes) {
	 ...
	 * 
	 * // Otherwise process try { PtreeWindow pTreeWinApp = new
	 * PtreeWindow(startPtreeRef);
